...
class AgentPoller {
  ...
  @Scheduled(fixedDelayString = "${agent.poll-ms}") void poll() {
    ...
    try {
      ...
      if (manifest.isEmpty() || !new AgentBoundary().accepts(manifest.get())) {
        if (manifest.isPresent()) result(manifest.get().deploymentId(), new DeploymentExecutor.ExecutionResult(false, "Agent rejected unsafe or expired deployment manifest.", "Manifest validation failed", null, null, null, null, List.of()));
        return;
      }
      DeploymentExecutor.ExecutionResult executed=executor.execute(manifest.get(),line->log(manifest.get().deploymentId(),line)); reportPlan(manifest.get().deploymentId(), executed.services()); log(manifest.get().deploymentId(),executed.message()); result(manifest.get().deploymentId(),executed);
  ...
  private void log(String deploymentId,String message) { try { String body=json.writeValueAsString(Map.of("messages",List.of(message))); http.sendAsync(request("/api/v1/agent/"+identity.serverId()+"/deployments/"+deploymentId+"/logs").header("Content-Type","application/json").POST(HttpRequest.BodyPublishers.ofString(body,StandardCharsets.UTF_8)).build(),HttpResponse.BodyHandlers.discarding()); stage(deploymentId,message); } catch(Exception ignored){} }
  private void stage(String deploymentId,String message){String status=message.startsWith("Checking out")?"CHECKOUT":message.startsWith("Static plan")?"ANALYZING":message.startsWith("Building immutable")?"BUILDING":message.startsWith("Starting isolated")?"DEPLOYING":message.startsWith("Starting health check")?"HEALTH_CHECKING":null;if(status==null)return;try{String body=json.writeValueAsString(Map.of("status",status));http.sendAsync(request("/api/v1/agent/"+identity.serverId()+"/deployments/"+deploymentId+"/stage").header("Content-Type","application/json").POST(HttpRequest.BodyPublishers.ofString(body,StandardCharsets.UTF_8)).build(),HttpResponse.BodyHandlers.discarding());}catch(Exception ignored){}}
  private void reportPlan(String deploymentId,List<DeploymentExecutor.ServicePlan> services) { if (services.isEmpty()) return; try { String body=json.writeValueAsString(Map.of("services",services)); http.send(request("/api/v1/agent/"+identity.serverId()+"/deployments/"+deploymentId+"/plan").header("Content-Type","application/json").POST(HttpRequest.BodyPublishers.ofString(body,StandardCharsets.UTF_8)).build(),HttpResponse.BodyHandlers.discarding()); } catch(Exception ignored){} }
