...
class AgentPoller {
  ...
  private void result(String deploymentId, boolean success, String message, String reason) { try { String body = json.writeValueAsString(Map.of("success", success, "message", message, "reason", reason == null ? "" : reason)); http.sendAsync(request("/api/v1/agent/" + identity.serverId() + "/deployments/" + deploymentId + "/result").header("Content-Type", "application/json").POST(HttpRequest.BodyPublishers.ofString(body, StandardCharsets.UTF_8)).build(), HttpResponse.BodyHandlers.discarding()); } catch (Exception ignored) { } }
